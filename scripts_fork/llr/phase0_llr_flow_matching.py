...
This is a LOSS, not a log-likelihood. **Positive = the gold CoC fits the gold action BETTER.**
``phase0_llr_per_token.py``'s ``llr`` is a log prob where positive also means "gold helps", so the
two agree in sign but NOT in units: this is a squared-error gap, not nats. Do not put the two in
one table, and do not call this number an LLR in nats. It is a negative-log-likelihood SURROGATE:
the unweighted flow-matching MSE is only proportional to an NLL bound under a particular t
weighting, which is not applied here. If a nats-denominated number is needed, confirm the sign on
a handful of events with the probability-flow ODE (instantaneous change of variables + Hutchinson
trace); that is expensive and is deliberately not implemented here.
...
Same venv/CWD rules as the rest of this directory (``a1x_rl_b300`` on this sm_103 box, run from a
recipe directory -- see ``README.md``).
...
loss. Everything downstream (content term, robust battery, Delta-vs-t curve) is computed from
that, so the analysis can be redone without re-running the model.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--t-sampler", default="stratified", choices=["stratified", "beta"],
        help="'stratified' is the ((j+0.5)/N) grid (variance reduction); 'beta' reproduces the "
             "training sampler (Beta(1.5,1) scaled) and is noisier.",
    )
    p.add_argument(
...
def main() -> None:  # noqa: C901 -- one linear measurement, splitting it hides the invariants
    ...
    print(f"[fm-llr] {N} CRN draws, t-sampler={args.t_sampler}, "
          f"t in [{t_all.min():.3f}, {t_all.max():.3f}]", flush=True)
    ...
    for a in arms:
        if a not in ("random", "cluster", "shuffled", "empty"):
            raise SystemExit(f"unknown arm {a!r}; choose from random/cluster/shuffled/empty")
    ...
    # ---------------- the two scorers ----------------
    def tokenize(data: dict, coc_text: str, n_copies: int = 1) -> dict:
        sample = {
    ...
    def score_fast(data: dict, coc_text: str) -> np.ndarray:
        """ONE VLM pass, then all N draws through the expert on a batch-expanded KV cache."""
        td = to_device({"td": dict(tokenize(data, coc_text, 1))}, "cuda")["td"]
        ...
        for s in range(0, N, args.chunk):
            ...
            with torch.no_grad(), torch.autocast("cuda", dtype=torch.bfloat16):
                ...
                per_row = ((target - pred) ** 2).float().mean(dim=tuple(range(1, pred.dim())))
            out[idx] = per_row.detach().cpu().numpy()
            del chunk_cache
        del cache, snap
        return out
    ...
    def score_slow(data: dict, coc_text: str, micro_bs: int = 4) -> np.ndarray:
        ...
        state: dict = {"idx": None, "per_row": None}
        ...
        def clfp(training_data, pred):
            ...
            state["per_row"] = ((target - pred) ** 2).float().mean(
                dim=tuple(range(1, pred.dim()))
            ).detach().cpu()
            return state["per_row"].mean()
    ...
    if args.verify:
        ...
        for name, txt in (("gold", ev["gold_coc"]), ("empty", "")):
            ...
            ts = time.time() - t0
            d = np.abs(f - s)
            rel = d / np.maximum(np.abs(s), 1e-12)
            print(
                f"[verify] {name:6s} max_abs={d.max():.3e} mean_abs={d.mean():.3e} "
    ...
    for i, ev in enumerate(sel):
        try:
            data = load_physical_aiavdataset(ev["clip_id"], t0_us=ev["t0_us"], avdi=avdi)
            conds: list[tuple[str, int, str]] = [("gold", -1, ev["gold_coc"])]
            if "empty" in arms:
                conds.append(("empty", -1, ""))
            if "random" in arms:
                for di, d in enumerate(draw_donors(ev["clip_id"], ev["event_idx"])):
                    conds.append(("donor", di, d))
            if "cluster" in arms:
                for di, d in enumerate(
                    draw_donors(ev["clip_id"], ev["event_idx"], cluster=ev["event_cluster"])
                ):
                    conds.append(("donor_cluster", di, d))
            if "shuffled" in arms:
                for di, d in enumerate(
                    draw_shuffled(ev["clip_id"], ev["event_idx"], ev["gold_coc"])
                ):
                    conds.append(("shuffled", di, d))
            for name, di, txt in conds:
                losses = score_fast(data, txt)
                for j in range(N):
                    rows.append(
                        {
